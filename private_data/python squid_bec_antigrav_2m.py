...
# Parameters for antigravity via 2m tunnel
L = 1.0
dx = 0.0001
x = np.arange(-1, 1 + dx, dx)
t_steps = 2000
k_wave = 0.0047  # 2m wave tunneling
epsilons = [0.9115]
phi1 = 12 * np.exp(-(x / L)**2)
phi2 = 0.5 * np.sin(k_wave * x)
lambda_d = 0.004
mass = 0.001
alpha = 50.0  # High coupling for antigravity
beta = 0.0025
...
try:
    for epsilon in epsilons:
        ...
        for t in range(t_steps):
            ...
            phi1_new = phi1_temp + dt_adaptive * (-k_damp * grad_phi2 * phi1_temp - alpha * phi1_temp * phi2_temp * np.cos(k_wave * np.abs(x)) * f_ALD)  # Negated coupling
            ...
            delta_m_t = epsilon * phi1_phi2**2 * mass * (eta / 1e9) * -1  # Negative perturbation
    ...
    # Save results
    output_file = 'squid_bec_antigrav_2m_results.txt'
